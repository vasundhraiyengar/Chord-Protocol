defmodule Project3 do
  ...
  def main(args) do
    ...
    map =
      Enum.reduce(pid_collection, %{}, fn x, acc ->
    ...
    cond do
      numNodes <= 100 -> :timer.sleep(1000)
      100 < numNodes && numNodes <= 400 -> :timer.sleep(10000)
  ...
  def handle_call(
        :Get_Hash,
        _from,
        {finger_list, my_hash, predecessor, successor, hop_count, finger_count}
      ) do
    hash = my_hash
  ...
  def set_succ_pre(
    ...
    GenServer.cast(
      pid,
      {:Set_Succ_Pre, successor1, successor1_hash, predecessor1, predecessor1_hash,
       hash1_collection}
    )
  end
  ...
  def find_successor(self_pid, my_hash, finger_list, successor, id, from_pid) do
    ...
    if id == my_hash do
      ...
      if (id > my_hash && id <= successor[:id]) || id == successor[:id] ||
           (id < my_hash && id <= successor[:id]) do
  ...
  def handle_call(
        {:Find_Successor_Count, id, from_pid},
        _from,
        {finger_list, my_hash, predecessor, successor, hop_count, finger_count}
      ) do
  ...
  def handle_cast(
    ...
    finger_list_up =
      for i <- 0..(m - 1) do
        ...
        result =
          if (id > my_hash && id <= y_h) || id == y_h do
            successor
          else
            cpn = closest_preceeding_node_try(finger_list, my_hash, id, successor)
